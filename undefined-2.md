...
자바스크립트 코드를 실행할 수 있는 런타임 플리케이션 입니다. 이 프로젝트에서는 JavaScript 패키지 매니저인 Yarn을 설치하고 프로젝트를 빌드하는 데에 사용합니다.
...
기존의 Ant와 Maven과 같은 빌드 도구 입니다. JVM에서 동작하는 스크립트 언인 그루비를 기반으로 동작하기 때문에, **** 다른 빌드 도구에 비해 쉽게 익힐 수 있습니다.

### **Spring Boot 2**

스프링 부트는 기존의 스프링 프레임워크를 기반으로 하는 프로젝트 입니다. 간단한 설정으로 실행 가능한 스프링 기반의 웹 플리케이션을 만드는 것 외에도, 자주 쓰이는 라이브러리의 호환성을 보장해 주는 것을 목표로 합니다.
...
서로 다른 객체 형식의 필드 값들을 자동으로 매핑 시켜주는 라이브러리 입니다. 여기서는 Entity와 DTO 객체를 매핑하는데 사용합니다.

### **Jackson**

Java에서 사용되는 가장 대표적인 객체 <-> 데이터 변환용 라이브러리 입니다. 여기서는 JSON 데이터로 가공하거나 JSON 데이터를 Java 객체로 변환하는 데에 사용합니다.

### **Lombok**

Java에서 반복적으로 작성하는 함수들을 Annotation을 통해 자동으로 작성되도록 도와주는 라이브러리 입니다.
